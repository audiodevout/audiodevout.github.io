...
const BackgroundCanvas = {
    ...
    createParticles() {
        ...
        const particleCount = Utils.isTouchDevice() ? 
            Math.min(this.maxParticles * 0.5, 60) : 
            this.maxParticles;
    ...
    drawParticles() {
        this.particles.forEach(particle => {
            ...
            // Enhanced rendering for bright particles
            const glowMultiplier = particle.type === 'bright' ? 2 : 1;
            const coreMultiplier = particle.type === 'bright' ? 0.8 : 0.5;
            ...
            this.ctx.shadowBlur = size * 3 * glowMultiplier;
            ...
            this.ctx.fillStyle = color;
            this.ctx.beginPath();
            this.ctx.arc(particle.x, particle.y, size * coreMultiplier, 0, Math.PI * 2);
            this.ctx.fill();
    ...
    drawConnections() {
        ...
        const strongConnectionDistance = 80;
        ...
        for (let i = 0; i < this.particles.length; i++) {
            for (let j = i + 1; j < this.particles.length; j++) {
                ...
                if (distance < maxConnectionDistance) {
                    ...
                    let lineWidth = 0.5;
                    let color = `hsla(180, 100%, 50%, ${alpha})`;
                    
                    // Stronger connections for closer particles
                    if (distance < strongConnectionDistance) {
                        lineWidth = 1;
                        const strongAlpha = (1 - distance / strongConnectionDistance) * 0.5;
                        color = `hsla(300, 100%, 50%, ${strongAlpha})`;
                    }
    ...
    drawNetworkConnections() {
        const networkDistance = 200;
        const connectionsPerParticle = 3;
        
        this.particles.forEach((particle, index) => {
            let connections = 0;
            
            for (let i = 0; i < this.particles.length && connections < connectionsPerParticle; i++) {
                if (i === index) continue;
                
                const other = this.particles[i];
                const distance = Utils.distance(particle.x, particle.y, other.x, other.y);
                
                if (distance < networkDistance) {
                    const alpha = (1 - distance / networkDistance) * 0.1;
                    
                    this.ctx.save();
                    this.ctx.globalCompositeOperation = 'screen';
                    this.ctx.strokeStyle = `hsla(60, 100%, 50%, ${alpha})`;
